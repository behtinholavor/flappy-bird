...
// Definição da var de podium
var podiumclickable = false;

// Definição da var de login
var loginclickable = false;
...
function showScore() {
   ...
   $("#scoreboard").transition({ y: '0px', opacity: 1 }, 600, 'ease', function () {
      ...
      $("#podium").transition({ opacity: 1 }, 600, 'ease');
      $("#login").transition({ opacity: 1 }, 600, 'ease');
...
$("#podium").click(function () {
   // Podemos deixar a ação de podium com clique também
   if (!podiumclickable) {
      window.location.href = 'ranking.html';
      return;
   } else {
      podiumclickable = false;
   }
});
...
function showLogin() {
   login = document.getElementById('loginboard');
